...
import com.example.collegeappandchatbot.Adapter.clsivAdapter;
import com.example.collegeappandchatbot.Adapter.dvivAdapter;
...
public class IndustrialVisit extends AppCompatActivity {
    ...
    dvivAdapter categoryAdapter;
    clsivAdapter Adapter;
    BottomNavigationView bottomNavigationView;
    List<industrivresponse> categorylist = new ArrayList<industrivresponse>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        classevents(); // Load class events by default
    ...
    private void classevents() {
        ...
        Call<clsivlist> call=api.getclsiv();

        call.enqueue(new Callback<clsivlist>() {
            @Override
            public void onResponse(Call<clsivlist> call, Response<clsivlist> response) {
                if (response.isSuccessful()) {
                    List<industrivresponse> categoryItems = response.body().getCategorylist();
                    ...
                    if (categoryItems != null && categoryItems.size() > 0) {
                        ...
                        Adapter = new clsivAdapter(getApplicationContext(),categorylist);
                        RecyclerView.LayoutManager mLayoutManager = new GridLayoutManager(IndustrialVisit.this,1);
            ...
            @Override
            public void onFailure(Call<clsivlist> call, Throwable t) {
    ...
    private void departmentevent(String department_id) {
        ...
        ApiService api = retrofit.create(ApiService.class);
        Call<dpivlist> call=api.getdiv(department_id);

        call.enqueue(new Callback<dpivlist>() {
            @Override
            public void onResponse(Call<dpivlist> call, Response<dpivlist> response) {
                if (response.isSuccessful()) {
                    List<industrivresponse> categoryItems = response.body().getCategorylist();
                    employee_list.removeAllViews();
                    ...
                    if (categoryItems != null && categoryItems.size() > 0) {
                        ...
                        categoryAdapter = new dvivAdapter(getApplicationContext(),categorylist);
                        RecyclerView.LayoutManager mLayoutManager = new GridLayoutManager(IndustrialVisit.this,1);
                        employee_list.setLayoutManager(mLayoutManager);
            ...
            @Override
            public void onFailure(Call<dpivlist> call, Throwable t) {
